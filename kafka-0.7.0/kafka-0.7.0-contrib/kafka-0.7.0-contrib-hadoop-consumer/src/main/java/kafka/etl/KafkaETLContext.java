...
public class KafkaETLContext {
    ...
    protected long[] getOffsetRange() throws IOException {
        ...
        if (startOffsets.length != 1)
            throw new IOException("input:" + _input + " Expect one smallest offset but get " + startOffsets.length);
    ...
    protected boolean hasError(ByteBufferMessageSet messages)
        ...
        if (errorCode == ErrorMapping.OffsetOutOfRangeCode()) {
            ...
            throw new IOException(_input + " current offset=" + _offset + " : invalid offset.");
            ...
            throw new IOException(_input + " current offset=" + _offset + " error:" + errorCode);
        } else
            return false;
    }
