...
trait BaseMessageSetTestCases extends JUnitSuite {
  ...
  def toMessageIterator(messageSet: MessageSet): Iterator[Message] = {
    ...
    val messages =JavaConversions.asJavaIterable(messageSet)
  ...
  def testIteratorIsConsistent() {
    ...
    TestUtils.checkEquals(JavaConversions.asJavaIterator(m.iterator), JavaConversions.asJavaIterator(m.iterator))
  }
