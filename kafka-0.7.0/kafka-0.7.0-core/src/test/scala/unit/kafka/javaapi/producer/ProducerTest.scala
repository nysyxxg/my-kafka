...
class ProducerTest extends JUnitSuite {
  ...
  def testZKSendToNewTopic() {
    ...
    try {
      ...
      producer.send(new ProducerData[String, String]("new-topic", "test",  Array("test1").toList ))
      Thread.sleep(100)
      producer.send(new ProducerData[String, String]("new-topic", "test",  Array("test1").toList))
      Thread.sleep(100)
  ...
  def testPartitionedSendToNewTopic() {
    ...
    syncProducer1.send("test-topic1", 0, new ByteBufferMessageSet(compressionCodec = NoCompressionCodec,
                                                                  messages =  Array(new Message("test1".getBytes)).toList))
    EasyMock.expectLastCall
    syncProducer1.send("test-topic1", 0, new ByteBufferMessageSet(compressionCodec = NoCompressionCodec,
                                                                  messages =  Array(new Message("test1".getBytes)).toList))
    EasyMock.expectLastCall
    ...
    producer.send(new ProducerData[String, String]("test-topic1", "test",  Array("test1").toList))
    Thread.sleep(100)
    ...
    producer1.send("test-topic1", new ByteBufferMessageSet(compressionCodec = NoCompressionCodec,
                                                           messages = Array(new Message("test".getBytes())).toList))
    Thread.sleep(100)

    // wait for zookeeper to register the new topic
    producer.send(new ProducerData[String, String]("test-topic1", "test1",  Array("test1").toList))
    Thread.sleep(100)
    producer.close
