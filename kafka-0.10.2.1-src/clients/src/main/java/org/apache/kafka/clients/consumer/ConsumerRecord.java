...
public class ConsumerRecord<K, V> {
    ...
    private final String topic; //消息的topic
    private final int partition; // 消费所在分区编号
    private final long offset; // 消息的topic
    private final long timestamp; //表示时间戳，与此对应的timestampType表示时间戳的类型
    private final TimestampType timestampType; // 有两种时间类型：CreateTime 和 LogAppendTime, 分别标识代表消息的创建的时间戳和消息追加到日志的时间戳。
    private final long checksum;  // 是CRC32的校验值。
    private final int serializedKeySize; // 表示key经过序列化之后的大小，如果key为空，则serializedKeySize的值为 -1。
    private final int serializedValueSize; //表示value经过序列化之后的大小，，如果value的值为空，则则serializedKeySize的值为 -1。
    private final K key;
