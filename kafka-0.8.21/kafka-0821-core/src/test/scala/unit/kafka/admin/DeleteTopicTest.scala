...
class DeleteTopicTest extends JUnit3Suite with ZooKeeperTestHarnessV2 {
  ...
  def testAddPartitionDuringDeleteTopic() {
    ...
    val servers = createTestTopicAndCluster(topic)
    // start topic deletion
    AdminUtils.deleteTopic(zkClient, topic) // 删除topic
    // add partitions to topic
    val newPartition = TopicAndPartition(topic, 1) // 
  ...
  def createTestTopicAndCluster(topic: String): Seq[KafkaServer] = {
  ...
  def createTestTopicAndCluster(topic: String, brokerConfigs: Seq[Properties]): Seq[KafkaServer] = {
    val expectedReplicaAssignment = Map(0 -> List(0, 1, 2))
    val topicAndPartition = TopicAndPartition(topic, 0)
    // create brokers
  ...
  def verifyTopicDeletion(topic: String, servers: Seq[KafkaServer]) {
    val topicAndPartition = TopicAndPartition(topic, 0)
