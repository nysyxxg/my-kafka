...
private[kafka] class ZookeeperConsumerConnector(val config: ConsumerConfig,
  ...
  /**
   * 运用ZookeeperConsumerConnector类创建多线程并行消费测试类，ConsumerGroupExample类初始化，调用createMessageStreams方法，
              实际是在consume方法处理的逻辑，创建KafkaStream,以及阻塞队列(LinkedBlockingQueue),KafkaStream与队列个数一一对应，
              消费者线程数量决定阻塞队列的个数。registerConsumerInZK()方法：设置消费者组，注册消费者信息consumerIdString到zookeeper上。
   */
  def consume[K, V](topicCountMap: scala.collection.Map[String,Int], keyDecoder: Decoder[K], valueDecoder: Decoder[V])
    ...
    registerConsumerInZK(dirs, consumerIdString, topicCount)// 注册消费者信息到zk中
  ...
  class ZKRebalancerListener(val group: String, val consumerIdString: String,
    ...
    /**
     * 客户端启动后会在消费者注册目录上添加子节点变化的监听ZKRebalancerListener，ZKRebalancerListener实例会在内部创建一个线程，
     *  这个线程定时检查监听的事件有没有执行（消费者发生变化）,如果没有变化则wait1秒钟，当发生了变化就调用 syncedRebalance 方法，
     *  去rebalance消费者。
     */
      ...
      if (brokers.size == 0) {
        ...
        if (isShuttingDown.get || !offsetFetchResponseOpt.isDefined)
          ...
          if(reflectPartitionOwnershipDecision(partitionOwnershipDecision)) {
            ...
            updateFetcher(cluster)// 修改并重启Fetchers  
            true
          } else {
            false
          }
    ...
    private def updateFetcher(cluster: Cluster) {
      ...
      fetcher match {
        case Some(f) =>
          f.startConnections(allPartitionInfos, cluster)  // 调用fetcher的startConnections方法，初始化Fetcher并启动它  
        case None =>
      }
    }
