...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(Map<String, Object> configs,
        ...
        try {
            ...
            // 发送的消息大小设置最大值，如果你发送的消息超过了这个值，就不能发送消息到服务器端，需要进行修改
            // 默认是1M。根据实际情况，进行修改
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG);
            // Producer发送消息有一个缓存区，设置缓冲区的大小，默认值32M
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);
            // 指定支持的压缩数据格式
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG));
            // 最大阻塞时间，Block 阻塞的意思，默认时间：60s
            //  当buffer满了，或者metadata获取不到（比如leader挂了）
            // 或者序列化没有完成，分区函数没有计算完成，等等情况下，最大阻塞时间默认60S
            ...
            // 创建一个核心组件，RecordAccumulator 缓存消息
            this.accumulator = new RecordAccumulator(logContext,
                    config.getInt(ProducerConfig.BATCH_SIZE_CONFIG),  // 每个批次大小是16K。默认是16K
                    this.compressionType,
                    lingerMs(config),  // 设置多长时间，将一个批次的数据发送出去
                    retryBackoffMs,
                    deliveryTimeoutMs,
                    metrics,
            ...
            // 判断kafka的集群元数据信息是否存在
            if (metadata != null) {
                // metadata 包含了kafka集群元数据信息，主要有：kafka集群的节点有哪些，有哪些topic，
                // 每个topic的分区有哪些，以及该分区对应的ISR列表分布在哪些节点
                // 获取meta其本质来说，需要给kafka集群发送kafka集群的网络请求
                this.metadata = metadata;
            } else {
                // 创建一个 metadata 对象
                this.metadata = new ProducerMetadata(retryBackoffMs,
                        // 设置获取kafka元数据最大时间间隔， 每隔5分钟，获取一次kafka集群的元数据信息
            ...
            this.sender = newSender(logContext, kafkaClient, this.metadata);  // 创建一个发送线程，业务代码的封装
            String ioThreadName = NETWORK_THREAD_PREFIX + " | " + clientId;
            //实现把业务代码与线程代码进行隔离，这样会显得层次清晰
            this.ioThread = new KafkaThread(ioThreadName, this.sender, true);// 对send线程进行包装
            this.ioThread.start();
            config.logUnused();
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
       ...
       // TODO 初始化一个重要的网络管理的组件
        /**
         * max.in.flight.requests.per.connection：默认是5
         * 发送数据的时候，其实是多个网络连接，每个网络连接可以忍受Producer端发送给broker消息后，消息没有响应的个数
         * 因为kakfa有重试机制，当消息发送失败，就会失败重试，所以有可能会造成数据乱序，如果要想保证有序，这个值要把设置为1
         *
         * connections.max.idle.ms ： 默认是9分钟，一个网络连接最大空闲多久，超过这个空闲时间，就关闭这个网络连接
         * 可以设置为-1，不回收连接，减少频繁的创建和销毁连接
         *
         * reconnect.backoff.ms: Socket 尝试重新连接指定主机的时间间隔
         * send.buffer.bytes ： socket 发送数据的缓存区的大小，默认值是128K
    ...
    /**
     * linger.ms
     * 上面比如我们设置batch size为32KB,但是比如有的时刻消息比较少,过了很久,
     * 比如5min也没有凑够32KB,这样延时就很大,所以需要一个参数. 再设置一个时间,到了这个时间,
     * 即使数据没达到32KB,也将这个批次发送出去. 比如设置5ms,就是到了5ms,大小没到32KB,也会发出去
     * 总结
