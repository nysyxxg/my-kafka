...
class GroupCoordinator(val brokerId: Int,
  ...
  private def doJoinGroup(group: GroupMetadata,
    ...
    group.inLock {
      if (!group.is(Empty) && (!group.protocolType.contains(protocolType) || !group.supportsProtocols(protocols.map(_._1).toSet))) {
        ...
        //todo: 如果group已经是一个Dead的状态的group表示这个group已经死亡，直接返回UNKNOWN_MEMBER_ID代码。
          case Dead =>
            ...
            responseCallback(joinError(memberId, Errors.UNKNOWN_MEMBER_ID))
            //todo: 如果当前的group的状态是正在准备执行Rebalance操作，
          // 根据memberId是否是UNKNOWN_MEMBER_ID值，执行新的member加入到group的操作或者执行对group中存在的member的更新操作。
          ...
          case CompletingRebalance =>
            ...
            //todo：第一次会进入到这个分支
          case Empty | Stable =>
            //如果memberId为空，第一次进来就为空
            if (memberId == JoinGroupRequest.UNKNOWN_MEMBER_ID) {
              // if the member id is unknown, register the member to the group
              //todo: 如果memberId是UNKNOWN_MEMBER_ID值表示consumer是一个新生成的consumer,执行添加成员并进行balance的操作。
              addMemberAndRebalance(rebalanceTimeoutMs, sessionTimeoutMs, clientId, clientHost, protocolType,
                protocols, group, responseCallback)
            } else {
              //获取对应的MemberMetadata
      ...
      if (!group.has(memberId)) {
        ...
        group.currentState match {
          ...
          case CompletingRebalance =>
            ...
            //todo: 确认是leader consumer发送过来的请求
            if (group.isLeader(memberId)) {
              info(s"Assignment received from leader for group ${group.groupId} for generation ${group.generationId}")

              // fill any missing members with an empty assignment
              //把挂掉的consumer消费分区策略置为空
  ...
  private def setAndPropagateAssignment(group: GroupMetadata, assignment: Map[String, Array[Byte]]) {
    ...
    //todo: 每个分区消费方案封装在对应的MemberMetadata 中
