...
object RequestChannel extends Logging {
  ...
  class Request(val processor: Int,
    ...
    def updateRequestMetrics(networkThreadTimeNanos: Long, response: Response) {
       ...
       * signal to noise ratio. When it comes to metrics, there ismaxWait little difference either way as we round the value
       * to the nearest long.
       */
...
class RequestChannel(val queueSize: Int) extends KafkaMetricsGroup {
  ...
  def sendResponse(response: RequestChannel.Response) {
    ...
    //todo: 获取对应的Processor线程
    val processor = processors.get(response.processor)
    // The processor may be null if it was shutdown. In this case, the connections
    // are closed, so the response is dropped.
    if (processor != null) {
      //todo: 添加响应到processor线程对应的队列中
