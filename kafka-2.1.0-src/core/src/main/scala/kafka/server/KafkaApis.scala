...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handle(request: RequestChannel.Request) {
    try {
      ...
      request.header.apiKey match {
        ...
        //todo: 接受follower拉取数据的请求，同时也接受consumer消费数据fetch的请求
        ...
        //todo: 获取coordinator的请求
        case ApiKeys.FIND_COORDINATOR => handleFindCoordinatorRequest(request)
        //todo: 获取consumer注册的请求，consumer添加到group中
        case ApiKeys.JOIN_GROUP => handleJoinGroupRequest(request)
        //todo: 接受consumer发送心跳的请求
        case ApiKeys.HEARTBEAT => handleHeartbeatRequest(request)
        case ApiKeys.LEAVE_GROUP => handleLeaveGroupRequest(request)
        //todo: 获取leader consumer 发送过来的消费分区方案的请求
  ...
  def handleProduceRequest(request: RequestChannel.Request) {
    ...
    if (authorizedRequestInfo.isEmpty)
      ...
      //todo: 把接受到的消息追加到磁盘文件中，这一块待会详细分析
  ...
  def handleFetchRequest(request: RequestChannel.Request) {
    ...
    if (fetchRequest.isFromFollower) {
      ...
      } else { //todo:  认证失败，对所有分区都构造错误响应加入 erroneous
      ...
      //todo： fetch 请求来自于客户端（消费者），和之前处理一样，认证失败或者分区不存在则构造错误响应
      fetchContext.foreachPartition { (topicPartition, data) =>
        if (!authorize(request.session, Read, Resource(Topic, topicPartition.topic, LITERAL)))
    ...
    if (interesting.isEmpty)
      ...
      //todo: 调用ReplicaManager.fetchMessages方法对 interesting 请求进行处理
      replicaManager.fetchMessages(
        fetchRequest.maxWait.toLong,   // 最大等待时间
        fetchRequest.replicaId,       // 副本 id，客户端为 Consumer 则为 -1
        fetchRequest.minBytes,       // 请求中的最小字节数
        fetchRequest.maxBytes,       // 请求中的最大字节数
        versionId <= 2,             //判断 API 版本以兼容旧版本请求
    ...
    if (findCoordinatorRequest.coordinatorType == FindCoordinatorRequest.CoordinatorType.GROUP &&
      ...
      def createResponse(requestThrottleMs: Int): AbstractResponse = {
        ...
        val responseBody = if (topicMetadata.error != Errors.NONE) {
          ...
          //todo：获取该分区 leader partition所在的节点
          ...
          coordinatorEndpoint match {
            case Some(endpoint) if !endpoint.isEmpty =>
              //todo：封装响应，其中把coordinator所在的主机也携带上
