...
class KafkaRequestHandler(id: Int,
  ...
  def run() {
    while (!stopped) {
      ...
      //todo: 从requestQueue队列中取出每一个Request对象
      ...
      req match {
        ...
        case request: RequestChannel.Request =>
          try {
            ...
            //todo: 通过KafkaApis对Request对象进行最终的处理
            apis.handle(request)
          } catch {
...
class KafkaRequestHandlerPool(val brokerId: Int,
  ...
  //todo:创建一个定长为8的数组存储KafkaRequestHandler线程，numThreads：num.io.threads默认值为8
  val runnables = new mutable.ArrayBuffer[KafkaRequestHandler](numThreads)

  //todo: 默认启动了8个线程，一般情况下生产环境中需要设置该参数，提升处理请求的能力
