...
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None,
  ...
  def startup() {
    ...
    try {
      ...
      if (canStartup) {
        ...
        /* start log manager */
        //todo: 初始化LogManager  这里其本质是使用到了 Object LogManager 它的apply
        ...
        //TODO: NIO服务端 SocketServer是接收客户端Socket请求连接、处理请求并返回处理结果的核心类
        socketServer = new SocketServer(config, metrics, time, credentialProvider)
        //启动SocketServer服务
        socketServer.startup(startupProcessors = false)

        /* start replica manager */
        ///todo: 初始化ReplicaManager
  ...
  /**
    * todo: advertised.listeners参数的作用就是将Broker的Listener信息发布到Zookeeper中，供Clients（Producer/Consumer）使用。
    * 如果配置了advertised.listeners，那么就不会将listeners配置的信息发布到Zookeeper中去了：
    * @return
    */
  private[server] def createBrokerInfo: BrokerInfo = {
    //获取配置文件中advertised.listeners,构建EndPoint对象
