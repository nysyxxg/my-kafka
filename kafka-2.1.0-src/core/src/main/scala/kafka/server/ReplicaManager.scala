...
class ReplicaManager(val config: KafkaConfig,
  ...
  //todo:follower partition是如何拉取leader partition数据的,
  ...
  def appendRecords(timeout: Long,  // 请求处理超时时间
    ...
    //todo: 判断acks参数是否合法 ，requiredAcks合法取值是-1，0，1，否则视为非法
    if (isValidRequiredAcks(requiredAcks)) {
      ...
      //todo: requiredAcks如果为 -1，需要保证所有的分区副本都写入成功
      if (delayedProduceRequestRequired(requiredAcks, entriesPerPartition, localProduceResults)) {
        // create delayed produce operation
        val produceMetadata = ProduceMetadata(requiredAcks, produceStatus)
        //todo   Kafka服务端在处理客户端的一些请求时，如果不能及时返回响应结果给客户端，
        //todo: 会在服务端创建一个延迟操作对象（ DelayedOperation），并放在延迟缓存中（ DelayedOperationPurgatory ）。
        //todo:  Kafka的延迟操作有多种：延迟的生产、延迟的响应、延迟的加入、延迟的心跳
        //todo: 创建DelayedProduce对象,超过timeout时间后这个操作会被认定为超时，并立刻返回，发送响应给客户端
        val delayedProduce = new DelayedProduce(timeout, produceMetadata, this, responseCallback, delayedProduceLock)

        // create a list of (topic, partition) pairs to use as keys for this delayed produce operation
        //todo: 遍历有哪些topic需要检查延迟操作是否完成
        val producerRequestKeys = entriesPerPartition.keys.map(new TopicPartitionOperationKey(_)).toSeq

        // try to complete the request immediately, otherwise put it into the purgatory
        // this is because while the delayed produce operation is being created, new
        // requests may arrive and hence make this operation completable.
        //todo: 尝试执行 delayedProduce 的 tryComplete，执行成功则返回，失败则加入到哈希轮定时器
         //数据写入成功了，会唤醒处于时间轮中wait的线程，立马给响应到客户端。
      ...
      val responseStatus = entriesPerPartition.map { case (topicPartition, _) =>
        //todo: 封装响应结果  ---> INVALID_REQUIRED_ACKS  无效的ack参数异常，生产者会捕获到该异常
        topicPartition -> new PartitionResponse(Errors.INVALID_REQUIRED_ACKS,
          LogAppendInfo.UnknownLogAppendInfo.firstOffset.getOrElse(-1), RecordBatch.NO_TIMESTAMP, LogAppendInfo.UnknownLogAppendInfo.logStartOffset)
      }
      //todo: 最后调用了回调函数，构造INVALID_REQUIRED_ACKS异常并封装进回调函数调用中
  ...
  private def appendToLocalLog(internalTopicsAllowed: Boolean,
    ...
    entriesPerPartition.map { case (topicPartition, records) =>
      ...
      //todo: 如果要写入数据的topic是内部的topic    名称为：__consumer_offsets
      //consumer---> offset ----> 0.8kafka(ZK)
      //consumer ----> offset ----> 0.8以上的kafka(offset保存在内置的__consumer_offsets主题上)
      if (Topic.isInternal(topicPartition.topic) && !internalTopicsAllowed) {
        (topicPartition, LogAppendResult(
          LogAppendInfo.UnknownLogAppendInfo,
          Some(new InvalidTopicException(s"Cannot append to internal topic ${topicPartition.topic}"))))
      } else { //todo: 我们自己创建topic
        try {
          //todo: 获取对应的分区对象
          val partition = getPartitionOrException(topicPartition, expectLeader = true)
          ...
          //todo: 找不到 topic 分区对应的 Partition 对象
          case e@ (_: UnknownTopicOrPartitionException |
                   _: NotLeaderForPartitionException |
   ...
   * the callback function will be triggered either when timeout or required fetch info is satisfied
    *
    * todo: 从 leader 副本取得消息，等待足够数据可以获取。一旦超时或者请求条件被满足则回调被调用。
   */
  def fetchMessages(timeout: Long,
    ...
    //todo: replicaId >= 0 (Follower) 则为 true
    ...
    def readFromLog(): Seq[(TopicPartition, LogReadResult)] = {
      ...
      val result = readFromLocalLog(
        ...
        quota = quota)
       //TODO: 更新缓存在leader partition所在的broker节点上的副本LEO
    ...
    //todo: 请求不想等待 or 请求消息为空 or 读取的总字节数超过了最小积攒字节数 or 存在读取错误
    // 此时直接生成结果给回调函数处理
    if (timeout <= 0 || fetchInfos.isEmpty || bytesReadable >= fetchMinBytes || errorReadingData) {
      ...
      //todo: 否则就构建一个延迟操作等待条件满足
      // 可能是没有获取到数据，由于生产者长时间没有发送数据，那么leaderPartition是没有数据，那么followerPartition也是获取不到数据
      /**
        *   如果没有接受到数据的话，立马返回给生产者，然后生产者在发送数据，这样的话，会浪费很多的网络资源
        *   这里就可以了构建一个基于时间轮的延迟调度操作
        */
      ...
      val fetchPartitionStatus = logReadResults.map { case (topicPartition, result) =>
        ...
        //   logStartOffset: Long  follower 第一个可用 offset
        //   maxBytes: Long        响应中累积的最大字节数,
        ...
        // fetchInfo: PartitionData 来自客户端的请求字段
        //result.info.fetchOffsetMetadata封装了leader partition HW
        (topicPartition, FetchPartitionStatus(result.info.fetchOffsetMetadata, fetchInfo))
      }

      // 输入参数构造 FetchMetadata 对象
      val fetchMetadata = FetchMetadata(fetchMinBytes, fetchMaxBytes, hardMaxBytesLimit, fetchOnlyFromLeader,
        fetchIsolation, isFromFollower, replicaId, fetchPartitionStatus)

      //todo: 封装延时拉取操作任务,等待拉取到足够数量的消息， 当延时拉取操作执行时，会再读取一次日志文件，然后将拉取结果返回给follower副本。
      //这里延迟调度的时间是0，一般来说我们是需要设置该值， 比如100ms ，100ms后执行然后返回响应
      ...
      // 尝试立刻完成请求, 否则将其放入 purgatory时间轮，它是一个高效的延时队列,或者说定时器,
      // 因为每次创建延迟 fetch 操作时, 新的请求可能到达并使其可完成
      //todo: 把延迟调度的任务放到时间轮中进行调度
    ...
    try {
        ...
        try {
          metadataCache.getAliveBrokers.find(_.id == newLeaderBrokerId) match {
            ...
            case Some(_) =>
              ...
              else  //这个 partition 的本地副本已经是 follower 了
      ...
      if (isShuttingDown.get()) {
        ...
        val partitionsToMakeFollowerWithLeaderAndOffset = partitionsToMakeFollower.map { partition =>
         ...
         //todo： 添加了followPartition 向leader partition 拉取数据的任务
  ...
  private def maybeShrinkIsr(): Unit = {
    ...
    //todo：遍历该节点所有的分区ISR列表，
    // replicaLagTimeMaxMs默认为 (replica.lag.time.max.ms)10s
      ...
      nonOfflinePartition(topicPartition) match {
        case Some(partition) =>
          partition.getReplica(replicaId) match {
            case Some(replica) =>
              //todo：核心方法  LeaderPartition维护的所有该分区的副本的LEO值
  ...
  protected def createReplicaFetcherManager(metrics: Metrics, time: Time, threadNamePrefix: Option[String], quotaManager: ReplicationQuotaManager) = {
    //todo：构建ReplicaFetcherManager对象
