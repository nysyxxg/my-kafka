...
class ReplicaFetcherThread(name: String,
  ...
  override def processPartitionData(topicPartition: TopicPartition,
    ...
    //todo: 写数据到followPartition的磁盘文件，并且更新自己的LEO, 底层是跟leader Partition写数据到磁盘的代码一样
    val logAppendInfo = partition.appendRecordsToFollowerOrFutureReplica(records, isFuture = false)

    if (isTraceEnabled)
      trace("Follower has replica log end offset %d after appending %d bytes of messages for partition %s"
        .format(replica.logEndOffset.messageOffset, records.sizeInBytes, topicPartition))
    //todo：获取到最新的HW , 使用min(自己的LEO,leaderHW)计算出最新的HW
    ...
    partitionMap.foreach { case (topicPartition, fetchState) =>
      ...
      if (fetchState.isReadyForFetch && !shouldFollowerThrottle(quota, topicPartition)) {
        try {
          ...
          //todo:封装请求参数，在PartitionData对象中，封装了该副本的LEO = fetchOffset
