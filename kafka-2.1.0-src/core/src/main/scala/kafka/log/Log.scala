...
class Log(@volatile var dir: File,
  ...
  //java juc下面的一个数据结构
  // 基于跳表实现的并发安全的集合
  // 文件名作为key，value就是这个LogSegment对象
  //目的：就是为了根据offset快速定位到对应的LogSegment
  ...
  private def append(records: MemoryRecords, isFromClient: Boolean, assignOffsets: Boolean, leaderEpoch: Int): LogAppendInfo = {
    maybeHandleIOException(s"Error while appending records to $topicPartition in dir ${dir.getParent}") {
      //todo: 1、校验数据，主要是检查消息的大小及 crc 校验
      ...
      lock synchronized {
        ...
        if (assignOffsets) {
          // assign offsets to the message set
           //获取当前Log的LEO，即下一条写入到Log的msg的offset
          val offset = new LongRef(nextOffsetMetadata.messageOffset)
          // 将分配的offset置为firstOffset
          appendInfo.firstOffset = Some(offset.value)
          val now = time.milliseconds
          //对message 做进一步的验证：消息格式转换，调整magic的值，修改时间戳等操作，并为message分配offset
         ...
         // 获取验证后的消息
          ...
          //在新版的 Kafka 中，每条 msg 都会有一个对应的时间戳记录，
          // producer 端可以设置这个字段 message.timestamp.type 来选择 timestamp 的类型，默认为创建时间，如果设置为日志追加时间
          //则将logAppendTime设置为当前时间
        ...
        //todo: 7、根据条件判断，把内存中的数据写入到磁盘中，这一块其实是不会走的
        //def unflushedMessages: Long = this.logEndOffset - this.recoveryPoint
        if (unflushedMessages >= config.flushInterval)

        //默认永远不会执行该操作，而是把内存中的数据写磁盘交给操作系统去管理
        //操作系统中的机制定期把数据写到磁盘中

        //但是这里我们是可以配置定时刷到磁盘的时间，比如5分钟，这样的话就会每个5分钟执行一次
          flush()
  ...
  private def trimInvalidBytes(records: MemoryRecords, info: LogAppendInfo): MemoryRecords = {
    ...
    if (validBytes == records.sizeInBytes) {
      ...
      // trim invalid bytes 从该消息集的末尾删除任何无效字节
  ...
  def read(startOffset: Long,
    ...
    maybeHandleIOException(s"Exception while reading from $topicPartition in dir ${dir.getParent}") {
      ...
      while (segmentEntry != null) {
        ...
        //todo: 通过LogSegment读取磁盘数据，数据封装在FetchDataInfo对象
   ...
   */
  //todo: 使用传入的函数计算哪些日志段对象能够被删除；调用 deleteSegments 方法删除这些日志段。
    ...
    maybeHandleIOException(s"Error while deleting segments for $topicPartition in dir ${dir.getParent}") {
      ...
      if (numToDelete > 0) {
        ...
        lock synchronized {
          ...
          // 如果我们删除了日志段对象，很有可能对外可见消息的范围发生了变化，自然要看一下是否需要更新 Log Start Offset 值。
          // 这就是 deleteSegments 方法最后要更新 Log Start Offset 值的原因
  ...
  def deleteOldSegments(): Int = {
    if (config.delete) {
      //todo:  根据时间判断，根据空间判断，根据baseOffset < logStartOffset判断
  ...
  def flush(offset: Long) : Unit = {
    maybeHandleIOException(s"Error while flushing log for $topicPartition in dir ${dir.getParent} with offset $offset") {
      ...
      //todo:遍历当前主机的所有logSegments对象
      for (segment <- logSegments(this.recoveryPoint, offset))
      //todo: 调用了LogSement的flush方法
        segment.flush()
