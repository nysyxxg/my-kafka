...
class LogSegment private[log] (val log: FileRecords,
  ...
  //todo: 是否需要新建LogSegment对象
  def shouldRoll(rollParams: RollParams): Boolean = {
    val reachedRollMs = timeWaitedForRoll(rollParams.now, rollParams.maxTimestampInMessages) > rollParams.maxSegmentMs - rollJitterMs
      //（1）如果一个segment大小超过了1个G，就需要新建一个segment
    size > rollParams.maxSegmentBytes - rollParams.messagesSize ||
      //（2）距离上次创建日志分段的时间达到了一定的阈值（log.roll.hours）7天，并且数据文件有数据
      (size > 0 && reachedRollMs) ||
      //（3）索引文件满了（默认10M）  log.index.size.max.bytes
      offsetIndex.isFull ||
      //（4）时间索引文件满了（默认10M）
      timeIndex.isFull ||
      //（5）最大的 offset，其相对偏移量超过了正整数的阈值,追加的消息的偏移量与当前日志分段的偏移量之间的差值大于 Integer.MAX_VALUE
  ...
  def append(largestOffset: Long, //写入消息批次中消息的最大位移值
             largestTimestamp: Long, //写入消息批次中最大的时间戳
             shallowOffsetOfMaxTimestamp: Long,  //写入消息批次中最大时间戳对应消息的位移
             records: MemoryRecords): Unit = {  //真正要写入的的消息。MemoryRecords是Kakfa中Record在内存中的实现形式。
    //判断消息的大小
    if (records.sizeInBytes > 0) {
      trace(s"Inserting ${records.sizeInBytes} bytes at end offset $largestOffset at position ${log.sizeInBytes} " +
            s"with largest timestamp $largestTimestamp at shallow offset $shallowOffsetOfMaxTimestamp")
      // 获取FileRecord文件的末尾,它就是本次消息要写入的物理地址
      ...
      // append the messages
      // todo: 调用FileRecords的append添加records
      ...
      //todo: 这里并不是来一条数据就写一条索引，而是达到一定的条件才写一次索引，我们管它叫稀疏索引。
      //indexIntervalBytes 默认是4096 字节
      //也就是说每次写了4096个字节的消息就更新一次索引
      ...
      //添加消息的大小到bytesSinceLastIndexEntry变量中
