...
class Partition(val topicPartition: TopicPartition,
    ...
    inWriteLock(leaderIsrUpdateLock) {
      ...
      leaderReplicaIfLocal match {
        case Some(leaderReplica) =>
          ...
          //todo: !inSyncReplicas.contains(replica) 该副本不在ISR列表，并且该副本的LEO大于leaderHW
          //此时说该副本已经追上了Leader，可以添加到ISR列表中
  ...
  def maybeShrinkIsr(replicaMaxLagTimeMs: Long) {
    val leaderHWIncremented = inWriteLock(leaderIsrUpdateLock) {
      leaderReplicaIfLocal match {
        case Some(leaderReplica) =>
          //todo: 获取要移除出去的Replicas
          val outOfSyncReplicas = getOutOfSyncReplicas(leaderReplica, replicaMaxLagTimeMs)
          if(outOfSyncReplicas.nonEmpty) {
            //inSyncReplicas: ISR列表
            //outOfSyncReplicas :要移除出去的replica
            //todo:从ISR列表中移除掉满足条件的副本
            ...
            //todo: ISR列表发生变化后，LeaderHW值可能会发生变化，需要更新LeaderHW
            // LeaderHW=min(isr_allReplica_LEO)
  ...
  def getOutOfSyncReplicas(leaderReplica: Replica, maxLagMs: Long): Set[Replica] = {
     ...
     *
     **/
    //todo：获取在ISR列表中排除leaderReplica的其他副本
    val candidateReplicas = inSyncReplicas - leaderReplica
    //todo：过滤延迟的Replica
    //todo: 条件：time.milliseconds - r.lastCaughtUpTimeMs) > maxLagMs（默认10s）
    //todo:  当前时间 - 上一次同步数据的时间 > 最大延迟时间
    //也就是说一个replica长时间没有过来同步数据，就需要把它从ISR列表中移除掉
  ...
  def appendRecordsToLeader(records: MemoryRecords, isFromClient: Boolean, requiredAcks: Int = 0): LogAppendInfo = {
    val (info, leaderHWIncremented) = inReadLock(leaderIsrUpdateLock) {
      leaderReplicaIfLocal match {
        //todo：只有 leader 副本支持追加消息操作
        case Some(leaderReplica) =>
          //获取leader partition的对应的Log对象
          val log = leaderReplica.log.get
          //获取minInSyncReplicas参数（min.insync.replicas）,默认值为2
          val minIsr = log.config.minInSyncReplicas
          // ISR列表副本个数
          val inSyncSize = inSyncReplicas.size

          // Avoid writing to leader if there are not enough insync replicas to make it safe
          //如果 ISR列表中副本个数小于最小的ISR副本数并且ack为-1 ，直接抛出异常
          ...
          //todo:  往 leader 副本的 Log 对象中追加消息，本质是调用Log的append函数
  ...
  def readRecords(fetchOffset: Long,
    ...
    // todo: 决定是否仅从 leader 获取, 然而无论是 Consumer 还是 Follower 都会从 leader 获取
