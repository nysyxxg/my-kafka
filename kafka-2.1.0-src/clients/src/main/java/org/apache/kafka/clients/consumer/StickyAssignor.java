 ...
 *
 *   StickyAssignor分区分配算法，目的是在执行一次新的分配时，能在上一次分配的结果的基础上，尽量少的调整分区分配的变动，
 *   节省因分区分配变化带来的开销。Sticky是“粘性的”，可以理解为分配结果是带“粘性的”——每一次分配变更相对上一次分配做最少的变动。
 *   其目标有两点：
 *    1.分区的分配尽量的均衡。
 *    2. 每一次重分配的结果尽量与上一次分配结果保持一致。
 *   当这两个目标发生冲突时，优先保证第一个目标。第一个目标是每个分配算法都尽量尝试去完成的，
 *   而第二个目标才真正体现出StickyAssignor特性的。
 */
