...
public class FileRecords extends AbstractRecords implements Closeable {
    ...
    public void flush() throws IOException {
        //写消息到磁盘文件，底层采用了FileChannel来实现，出于性能方法的考虑，操作系统会将数据缓存在内存中（基于OS Cache），
        // 所以无法保证写入到FileChannel里的数据一定会即时写到磁盘上，要保证这一点，需要调用force方法
        //todo: 调用javaNIO的方法，强制把内存中的数据刷到磁盘中
        channel.force(true);
    }
