 ...
 * //todo: ConsumerCoordinator实现了AbstractCoordinator，主要负责和服务器端GroupCoordinator进行交互
 */
public final class ConsumerCoordinator extends AbstractCoordinator {
     ...
     * todo: 拉取协调器事件。这可以确保协调器是已知的，并且消费者已加入组（如果它使用组管理）。
     * 如果启用了周期性偏移提交，这也会处理这些提交。
     */
    public boolean poll(Timer timer) {
        ...
        if (subscriptions.partitionsAutoAssigned()) {
            ...
            // todo: 确保ConsumeCoordinator创建完成，如果没有则向服务器发送请求开始创建一个
            if (coordinatorUnknown() && !ensureCoordinatorReady(timer)) {
                return false;
            }

            //todo: 检查当前 consumer 是否需要加入 group。事实上 consumer 正是在调用 poll 方法的时候才加入到 group 里面的，
            // 如果你更改了 topic 的注册信息（如有 consumer 退出了当前 group），那么当前 consumer 就需要再次重新加入 group
            if (rejoinNeededOrPending()) {
                // due to a race condition between the initial metadata fetch and the initial rebalance,
                // we need to ensure that the metadata is fresh before joining initially. This ensures
                // that we have matched the pattern against the cluster's topics at least once before joining.
                //支持注册 topic 的时候可以使用正则匹配
                if (subscriptions.hasPatternSubscription()) {
                ...
                //todo: 当成功找到Coordinator后，那么就要发起加入group的请求，表示该consumer是该组的成员，Coordinator会接收到该请求，会给集群分配一个Leader
        ...
        //todo:  如果设置的是自动commit,会周期性的异步提交偏移量
    ...
    public void commitOffsetsAsync(final Map<TopicPartition, OffsetAndMetadata> offsets, final OffsetCommitCallback callback) {
        ...
        //todo: 检查coordinator是否正常，由于偏移量由kafka内置的topic维护，就是要找到该coordinator对应的主机是否正常,
        //偏移量就是要写入到coordinator所在的主机上
    ...
    private RequestFuture<Void> sendOffsetCommitRequest(final Map<TopicPartition, OffsetAndMetadata> offsets) {
        ...
        //todo: 封装一个提交偏移量的请求,ApiKeys为OFFSET_COMMIT
