...
public abstract class AbstractCoordinator implements Closeable {
     ...
     * todo: ensureCoordinatorReady() 中实现，主要的作用是发送 FindCoordinatorRequest 请求，并建立连接
     */
    protected synchronized boolean ensureCoordinatorReady(final Timer timer) {
              //获取到groupCoordinator了
            if (!coordinatorUnknown())
                return true;

        do {
            //todo: 核心代码, 找到GroupCoordinator，并建立连接
    ...
    protected synchronized RequestFuture<Void> lookupCoordinator() {
        if (findCoordinatorFuture == null) {
            ...
            if (node == null) {
                ...
                //todo:  发送FindCoordinator request请求
     ...
     * todo: ensureActiveGroup()这个方法的主要作用是向GroupCoordinator发送join-group、sync-group请求，
     * 获取assign的TopicPartition list。
     */
    ...
    RequestFuture<ByteBuffer> sendJoinGroupRequest() {
        ...
        return client.send(coordinator, requestBuilder, joinGroupTimeoutMs)
                //todo: 通过JoinGroupResponseHandler处理响应
    ...
    private class JoinGroupResponseHandler extends CoordinatorResponseHandler<JoinGroupResponse, ByteBuffer> {
        ...
        public void handle(JoinGroupResponse joinResponse, RequestFuture<ByteBuffer> future) {
            ...
            if (error == Errors.NONE) {
                ...
                //todo: 所有的消费者都会发送JoinGroup,最终只有1个consumer被指定为Leader Consumer,如果是leader consumer,它会调用下面的方法
    ...
    //leader consumer开始基于消费者线程数和topic的分区数，开始制定分区 消费方案
    private RequestFuture<ByteBuffer> onJoinLeader(JoinGroupResponse joinResponse) {
        try {
            ...
            //todo: 创建SyncGroupRequest，封装了分区消费方案
            SyncGroupRequest.Builder requestBuilder =
                    new SyncGroupRequest.Builder(groupId, generation.generationId, generation.memberId, groupAssignment);
            log.debug("Sending leader SyncGroup to coordinator {}: {}", this.coordinator, requestBuilder);
            //todo: 发送分区消费的方案请求给coordinator
            return sendSyncGroupRequest(requestBuilder);
    ...
    private RequestFuture<Void> sendFindCoordinatorRequest(Node node) {
        ...
        //todo: 封装FindCoordinatorRequest的请求，设置请求的ApiKeys为FIND_COORDINATOR
        FindCoordinatorRequest.Builder requestBuilder =
                new FindCoordinatorRequest.Builder(FindCoordinatorRequest.CoordinatorType.GROUP, this.groupId);
        //todo: 缓存获取Coordinator Group 请求
        return client.send(node, requestBuilder)
                     .compose(new FindCoordinatorResponseHandler());
    }

    //todo: 根据response返回的ip以及端口和GroupCoordinator建立连接
    private class FindCoordinatorResponseHandler extends RequestFutureAdapter<ClientResponse, Void> {

        @Override
        public void onSuccess(ClientResponse resp, RequestFuture<Void> future) {
            log.debug("Received FindCoordinator response {}", resp);
            //todo:findCoordinatorFuture清空
            ...
            if (error == Errors.NONE) {
                ...
                //异常信息为GROUP_AUTHORIZATION_FAILED
            } else if (error == Errors.GROUP_AUTHORIZATION_FAILED) {
                future.raise(new GroupAuthorizationException(groupId));
            } else {
    ...
    synchronized RequestFuture<Void> sendHeartbeatRequest() {
        ...
        //todo: 封装一个发送心跳的请求，ApiKeys为HEARTBEAT
