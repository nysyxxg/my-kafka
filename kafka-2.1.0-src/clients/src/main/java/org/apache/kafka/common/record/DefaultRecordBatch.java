 ...
 *
 * RecordBatch =>
 *  BaseOffset => Int64                 表示当前RecordBatch的起始位移。
 *  Length => Int32                      该消息批次的长度
 *  PartitionLeaderEpoch => Int32        分区leader版本号
 *  Magic => Int8                       消息格式的版本号，对于v2版本而言，magic等于2。
 *  CRC => Uint32                       CRC校验码，用来确认消息在传输过程中不会被篡改，
 *  Attributes => Int16                  消息属性
 *  LastOffsetDelta => Int32 // also serves as LastSequenceDelta   RecordBatch中最后一个Record的offset与first offset的差值
 *  FirstTimestamp => Int64             起始时间戳：batch中第一条消息的时间戳
 *  MaxTimestamp => Int64               最大时间戳：batch中最后一条消息的时间戳
 *  ProducerId => Int64                 用来支持幂等性。
 *  ProducerEpoch => Int16              表示producer携带的当前版本号，和producer id一样，用来支持幂等性。
 *  BaseSequence => Int32               和producer id、producer epoch一样，用来支持幂等性。
 *  Records => [Record]                存储在批次中的消息
 *
