...
public class Sender implements Runnable {
     ...
     *  //todo: 判断inFlightBatches中的Batch是否超时
     */
    private List<ProducerBatch> getExpiredInflightBatches(long now) {
        ...
        for (Map.Entry<TopicPartition, List<ProducerBatch>> entry : inFlightBatches.entrySet()) {
            ...
            //获取分区对应的ProducerBatch集合
            List<ProducerBatch> partitionInFlightBatches = entry.getValue();
            //partitionInFlightBatches不为空
            if (partitionInFlightBatches != null) {
                ...
                while (iter.hasNext()) {
                    ...
                    //判断批次是否超时  判断是否过期的标准是系统当前时间与 ProducerBatch 创建时间之差是否超过120s
        ...
        if (transactionManager != null) {
            try {
                ...
                if (!transactionManager.isTransactional()) {
                    ...
                    // todo: 为idempotent producer生成一个producer id
    ...
    private long sendProducerData(long now) {
         ...
         *   按照brokerId进行分组，同一个broker的partition为同一组
         *  key    value
         *  0      p0,p1
         *  1      p2
         *  2      p3
        ...
        //todo: 将待发送的ProducerBatch封装成为ClientRequest
    ...
    private void completeBatch(ProducerBatch batch, ProduceResponse.PartitionResponse response, long correlationId,
        ...
        /**
         * todo: 如果响应是'MESSAGE_TOO_LARGE'的错误，且是批量消息（recordCount>1）
         */
        ...
        } else if (error != Errors.NONE) {//todo: 如果带有异常
             //判断是否允许重试
    ...
    private void failBatch(ProducerBatch batch, long baseOffset, long logAppendTime, RuntimeException exception,
        ...
        /**
         * todo: 对于发送异常且不可重试的请求，也是调用消息的回调函数，并且释放内存池中的内存
         */
    ...
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<ProducerBatch> batches) {
        ...
        //遍历所有的batch消息，找到最新的版本号信息
        for (ProducerBatch batch : batches) {
            if (batch.magic() < minUsedMagic)
                minUsedMagic = batch.magic();
        }

        //todo: 遍历ProducerBatch集合，整理成ProducerBatch和MemoryRecords
        for (ProducerBatch batch : batches) {
        ...
        //todo: 生成ClientRequest对象，如果acks不等于0 ，则表示会等待服务端的响应
        ClientRequest clientRequest = client.newClientRequest(nodeId, requestBuilder, now, acks != 0,
                requestTimeoutMs, callback);
        //todo: 调用NetWorkClient的send方法。
        client.send(clientRequest, now);
