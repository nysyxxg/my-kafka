...
public final class RecordAccumulator {
    ...
    //todo: TopicPartition 分区-------》  Deque<ProducerBatch> 队列
    ...
    public    RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            //加锁
            //假设有3个线程进来了 ，都是需要把数据写往同一个分区中相同的队列中
            synchronized (dq) {
                //线程1先获得锁
                //线程2获取锁
                if (closed)
    ...
    public ReadyCheckResult ready(Cluster cluster, long nowMs) {
        ...
        //// exhausted记录是否有其他的线程再等待释放空间,如果exhausted为true，说明内存池中的内存不够了
        // exhausted它会作为一个是否要把该批次发送出去的条件
            ...
            synchronized (deque) {
                ...
                if (leader == null && !deque.isEmpty()) {
                    ...
                    if (batch != null) {
                         ...
                         *  backingOff 应该为false
                         *
                         *  还需要等多久timeToWaitMs=lingerMs 默认就是0 ，表示不需要等待
    ...
    private Deque<ProducerBatch> getOrCreateDeque(TopicPartition tp) {
        ...
        Deque<ProducerBatch> d = this.batches.get(tp);   //读多
        ...
        // 如果已经存在对应的值，则依旧为原来的值。
        Deque<ProducerBatch> previous = this.batches.putIfAbsent(tp, d);   //写少-----》每次调用针对于每一个分区而言
        if (previous == null)
            return d;
        else
            //直接返回previous
